...
| `--renderer inline\|fullscreen\|auto` | Picks the transcript renderer | `inline` prints finished blocks into your terminal's own scrollback (native selection/right-click/wheel/search keep working, like Claude Code); `fullscreen` uses the alt-screen viewport with in-app scrollbar and mouse capture. `auto` (default) picks inline on Termux and fullscreen elsewhere. Also settable via `REASONIX_RENDERER` or `ui.renderer` in config (flag > env > config). |
