...
| Transcript text selection | Copies transcript text | Fullscreen renderer: the TUI enables mouse reporting, so drag in the transcript to select text in-app; releasing the mouse copies it automatically, and `Ctrl+C`/`Super+C`/`Meta+C` or right-clicking the active selection copy it again. Inline renderer: your terminal's native selection, right-click menu, and clipboard work directly. |
| `/mouse` | Toggles in-app mouse capture (fullscreen renderer) | Off hands the mouse back to your terminal, restoring its native click-drag selection and right-click context menu, at the cost of in-app drag-select, the transcript scrollbar, and wheel-scroll. Set `REASONIX_DISABLE_MOUSE=1` to start every session with it off. Under the inline renderer the terminal always owns the mouse, so `/mouse` just reminds you of that. |
...
| `Ctrl+B` | Expands or collapses long shell output | Fullscreen renderer: long shell-output hint lines can also be clicked in the transcript, and text selection is handled in-app while mouse reporting is enabled. Inline renderer: the expanded output is appended below (scrollback lines cannot be rewritten in place). |
| `--renderer inline\|fullscreen\|auto` | Picks the transcript renderer | `inline` (the default) prints finished blocks into your terminal's own scrollback — native selection, right-click, wheel scrolling, and search keep working, like Claude Code — while a pinned bottom region streams live progress. `fullscreen` uses the alt-screen viewport with the in-app scrollbar, drag-select, and mouse capture. Also settable via `REASONIX_RENDERER` or `ui.renderer` in config (flag > env > config). |
